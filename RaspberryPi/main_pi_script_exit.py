...
try:
    ...
    sys.stderr.write(f"Eroare la inițializarea LCD în main_pi_script_exit.py: {e}\n")
    lcd = None
...
def update_lcd_message(message1, message2=""):
    ...
    sys.stderr.write(f"LCD (main_pi_script_exit): {message1} / {message2}\n") # Pentru debugging prin SSH
...
def setup_gpio():
    ...
    sys.stderr.write("GPIO setat pentru iesire.\n")
...
def get_distance():
    ...
    # Wait for the echo to start (0)
    timeout_start = time.time()
    while GPIO.input(ECHO_PIN) == 0:
        start_time = time.time()
        if time.time() - timeout_start > 0.1: # Timeout for 0 state
            break

    # Wait for the echo to end (1)
    timeout_stop = time.time()
    while GPIO.input(ECHO_PIN) == 1:
        stop_time = time.time()
        if time.time() - timeout_stop > 0.1: # Timeout for 1 state
            break
...
def capture_image(picam2):
    ...
    picam2.start()
    time.sleep(0.5) # Dă timp camerei să se stabilizeze
...
def extract_license_plate(image_path):
    try:
        ...
        cv2.imwrite(f"{IMAGE_PATH}/debug_thresh_exit_{datetime.now().strftime('%Y%m%d_%H%M%S')}.jpg", thresh)
        
        sys.stderr.write(f"Număr de înmatriculare extras (OCR - Iesire): {cleaned_text}\n")
        return cleaned_text
    except Exception as e:
        sys.stderr.write(f"Eroare OCR (Iesire): {e}\n") 
        return None
...
def main():
    ...
    update_lcd_message("Iesire - Astept", "masina...") 
    ...
    try:
        ...
        while time.time() - start_time < DETECTION_TIMEOUT:
            ...
            if dist < DISTANTA_DETECTIE_CM:
                car_detected = True
                update_lcd_message("Iesire - Masina", "Detectata!")
                time.sleep(0.5)
                break
            time.sleep(SENSOR_POLLING_INTERVAL)

        if car_detected:
            update_lcd_message("Iesire - Extrag", "placa...")
            ...
            if license_plate_result:
                update_lcd_message(f"Iesire - Placa:", license_plate_result)
            else:
                update_lcd_message("Iesire - Placa", "NEDETECTATA")

        else:
            update_lcd_message("Iesire - Timeout", "Nicio masina")

    except Exception as e:
        sys.stderr.write(f"Eroare majoră în scriptul RPi de iesire: {e}\n")
        update_lcd_message("Eroare RPi Iesire", "Contactati admin")
    finally:
        if picam2_global:
            picam2_global.close()
        ...
        time.sleep(2) # Dă timp utilizatorului să vadă mesajul
        sys.stderr.write("main_pi_script_exit.py ended.\n")
        GPIO.cleanup() 
